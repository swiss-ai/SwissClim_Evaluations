...
def modify_ds(ds: xr.Dataset, path: str):
    ...
    target_path = (
        ...
        "IFS_ensmean_2020_totalprecipitation6hr.zarr"
    )
    if path == target_path:
        ds = ds.sel(
            prediction_timedelta=ds.prediction_timedelta.values[1:]
        )  # the first step is 0 hour and full of NaN

    if (
        path
        == "/capstor/store/cscs/swissai/a122/hydrological_data/FuXi_2020_totalprecipitation6hr.zarr"
    ):
