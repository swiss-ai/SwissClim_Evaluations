...
def run(
    ...
    multi_lead = (
        ...
        and "lead_time" in ds_prediction_std.dims
        and int(ds_prediction_std.lead_time.size) > 1
        and lead_policy.mode != "first"
    )
    if multi_lead:
        hours = (
            ds_prediction_std["lead_time"].values // np.timedelta64(1, "h")
        ).astype(int)
    ...
    for i, variable_name in enumerate(variables_2d):
        for lead_h in panel_hours:
            print(
                f"[wd_kde] variable: {variable_name}{' lead=' + str(lead_h) + 'h' if lead_h is not None else ''}"
            )
        ...
        for j in range(n_bands // 2):
            ...
            da_target_slice = ds_target_std[variable_name]
            da_prediction_slice = ds_prediction_std[variable_name]
            if lead_h is not None and "lead_time" in da_target_slice.dims:
                li = hour_to_idx.get(int(lead_h), 0)
                da_target_slice = da_target_slice.isel(lead_time=li)
            if lead_h is not None and "lead_time" in da_prediction_slice.dims:
                li = hour_to_idx.get(int(lead_h), 0)
                da_prediction_slice = da_prediction_slice.isel(lead_time=li)
            da_target_slice = da_target_slice.sel(
                latitude=slice(lat_min, lat_max)
            )
            da_prediction_slice = da_prediction_slice.sel(
                latitude=slice(lat_min, lat_max)
            )
            ...
            if ds_flat.size == 0 or ml_flat.size == 0:
                # Cannot compute distance or KDE; annotate and continue
                w = float("nan")
                axs[j, 1].set_title(
                    f"Lat {lat_min}° to {lat_max}° (Insufficient data)"
                )
            else:
        ...
        if save_fig:
            ...
            out_png = (
                section_output
                / f"{variable_name}_sfc_latbands_norm{lead_suffix}.png"
            )
        ...
        if save_npz:
            ...
            out_npz = (
                section_output
                / f"{variable_name}_sfc_latbands_kde_combined{lead_suffix}.npz"
            )
            np.savez(
                out_npz,
                mean_w=mean_w,
