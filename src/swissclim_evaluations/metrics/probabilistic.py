...
def run_probabilistic(
    ...
    for var in variables:
        c.print(f"[probabilistic] variable: {var}")
        ...
        for split_spec in split_specs:
            ...
            ds_p_slice = xr.Dataset({str(var): pred_slice})
            ds_t_slice = xr.Dataset({str(var): targ_slice})

            # Build small per-split lazy graphs (deterministic-style) instead of
            # slicing a single full-variable graph, which can become too large.
            ds_crps_slice = compute_wbx_crps(ds_p_slice, ds_t_slice, ensemble_dim="ensemble")
            crps_da = ds_crps_slice[var]
        ...
        if var_jobs:
            compute_jobs(
                ...
                desc=f"Computing CRPS and PIT metrics ({var})",
...
def plot_probabilistic(
    ...
    # Loop over ALL common variables to generate CRPS maps and line CSVs
    for var_name in common_vars:
        ...
        for lvl in levels:
            ...
            if "lead_time" not in ds_prediction.dims or ds_prediction.sizes["lead_time"] <= 1:
            ...
            if "lead_time" in crps_sub.dims and int(crps_sub.sizes.get("lead_time", 0)) > 1:
                ...
                if full_hours:
                    ...
                    # Sort lat/unwrap lon for crps_by_lead
                    if lat_vals.size > 1 and lat_vals[0] > lat_vals[-1]:
