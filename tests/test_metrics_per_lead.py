...
def _build_pair(n_leads=3):
    ...
    leads = (
        np.arange(n_leads).astype("timedelta64[h]").astype("timedelta64[ns]")
    )
    arr = xr.DataArray(
...
def test_deterministic_per_lead(tmp_path: Path):
    ...
    assert set([0, 1, 2]).issubset(set(df.lead_time_hours.unique()))
...
def test_ets_per_lead(tmp_path: Path):
    ...
    policy = LeadTimePolicy(mode="full")
    ets_mod.run(
        ds_t, ds_p, tmp_path, {"ets": {"thresholds": [50]}}, lead_policy=policy
    )
